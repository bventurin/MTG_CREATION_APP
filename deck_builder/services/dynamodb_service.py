...
class DynamoDBService:
    ...
    def get_user_decks(self, user_id):
        ...
        response = self.table.query(
            ...
            ExpressionAttributeValues={":pk": f"USER#{user_id}", ":sk": "DECK#"},
        )
    ...
    def get_deck_cards(self, deck_id, is_sideboard=None):
        ...
        response = self.table.query(
            KeyConditionExpression="pk = :pk AND begins_with(sk, :sk)",
            ExpressionAttributeValues={":pk": f"DECK#{deck_id}", ":sk": "CARD#"},
        )
    ...
    def update_deck(self, user_id, deck_id, deck_name, cards_data):
        ...
        self.table.update_item(
            ...
            ExpressionAttributeValues={":name": deck_name, ":updated": timestamp},
        )

        # Delete old cards
        old_cards = self.get_deck_cards(deck_id)
        for card in old_cards:
            self.table.delete_item(Key={"pk": card["pk"], "sk": card["sk"]})
    ...
    def delete_deck(self, user_id, deck_id):
        # Delete deck item
        self.table.delete_item(Key={"pk": f"USER#{user_id}", "sk": f"DECK#{deck_id}"})
