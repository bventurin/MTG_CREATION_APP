...
To ensure that AetherFlow remains reliable and maintainable as it grows, I implemented a fully automated CI/CD pipeline using GitHub Actions. Rather than relying on manual checks, every push to the repository triggers a multi-stage workflow designed to catch errors before they even reach a live environment.

The process begins with a **Testing phase**, where the pipeline spins up a Python environment and runs Django's internal system checks. This acts as our first line of defense, ensuring that basic configuration errors or dependency issues don't slip through.

Building on this, I integrated **SonarCloud** for advanced static analysis. This was a crucial addition for long-term code health. SonarCloud automatically scans the codebase for "code smells," potential bugs, and security vulnerabilities. For instance, during setup, it helped identify a hardcoded secret key fallback—a common security pitfall—which I was then able to pivot into a more secure, dynamically generated solution. This automated auditing ensures that the project doesn't just "work," but adheres to professional security and quality standards.

Once the code passes these quality gates, the **Deployment phase** takes over. Using GitHub Secrets to handle sensitive AWS credentials, the pipeline dynamically updates environment variables on the Elastic Beanstalk server (`AetherFlow-env`), packages the latest application state, and deploys it seamlessly. This transition from "code on a machine" to "live service" happens without any manual intervention, allowing for fast, confident iterations.

# Section 6: Conclusions and Findings

The main goal of this project was successfully achieved. The AetherFlow application allows users to securely create accounts, manage complex card collections, and receive intelligent AI-driven suggestions for their decks. It also provides technical features such as mana curve analytics and the generation of QR codes for easy sharing of their decks. These core functionalities were made possible through the programmatic integration of multiple external APIs and third-party web services—specifically the Scryfall API for card data and the Google Gemini API for card recommendations—which is the core functionality of the system.

### 6.1 Findings and Interpretations
Due to the robust documentation provided by AWS and the APIs, most of the services utilized were straightforward to configure. However, implementing the integration with the Scryfall API and the decentralized card synchronization service proved to be significantly more complex than initially anticipated. In particular, I found that ensuring the application respected API rate limits while syncing card data that was not previously found in the S3 bucket was a critical technical hurdle. This interpretation led to the realization that a redundant storage strategy—using S3 as a primary bulk-data cache—is essential for building resilient applications that rely on third-party data providers. Using this hybrid approach ensured the application remained responsive even when external services were under heavy load.

### 6.2 Personal Reflection
Building AetherFlow has been a profound learning experience. It forced me to shift my perspective from creating an isolated application to coordinating a distributed system of cloud-native modules. Transitioning to an event-driven mindset, where services like Scryfall sync or QR code generation run independently, offered great insight into how modern, scalable software is built. Seeing these separate pieces—Django, Lambda, DynamoDB, and various APIs—work together seamlessly was incredibly rewarding and reinforced the importance of robust automation and clear architectural design.

### 6.3 Future Work
If this application is to continue to be developed, integrating a premium payment option (for example, PayPal or Stripe) for specialized AI features and adding social deck-sharing components would make it even more functional for the community. Additionally, having a team to work on this project would have helped with brainstorming alternative architectural designs and allowing the application to be more robust through collaborative code reviews. If I were starting the project again, I would have utilized Infrastructure as Code (IaC) from the very beginning to allow for a quicker and more reproducible environment setup.
